...
public class MultiThreadingTest {
    ...
    public static void main(String[] args) {
        ...
        executor.submit(() -> {
            for (int i = 0; i < 5; i++) {
                simulateConcurrentThreads();
            }
        });

        // Shutdown the executor after a certain time
    ...
    private static void simulateConcurrentThreads() {
        // Simulate multiple threads querying the database concurrently
        new Thread(() -> {
            monitorMySQLLocks();
        }).start();
